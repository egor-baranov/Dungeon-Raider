...
{
    public class RuleTileWithSiblings<T> : RuleTile
    {
    ...
    public class RuleTileWithSiblings : TileBase
    {
        ...
        public virtual bool RuleMatch(int neighbor, TileBase tile)
        {
            switch (neighbor)
            {
                case TilingRule.Neighbor.This: return IsTileOneOfThese(this, m_Self) || IsTileOneOfThese(this, m_siblings);
                case TilingRule.Neighbor.NotThis: return (!IsTileOneOfThese(this, m_Self) && !IsTileOneOfThese(this, m_siblings));
            }
            return true;
        }
        ...
        private static bool IsTileOneOfThese(TileBase tile, params TileBase[] others)
        {
            if (tile == null)
            {
                return false;
            ...
            foreach (var otherTile in others)
            {
                // Ignore nulls
                if (otherTile == null)
                {
